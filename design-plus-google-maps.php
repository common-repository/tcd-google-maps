<?php
/*
Plugin Name: TCD Google Maps
Plugin URI: https://tcd-theme.com/
Description: Easily add Google Maps support!
Version: 1.9
Author: DesignPlus
Author URI: https://tcd-theme.com/wp-tcd
License: GPL
Copyright: DesignPlus
Text Domain: tcd-google-maps
*/
...
function tcd_script_enqueue() {
  global $hook_suffix;
  if ($hook_suffix != 'toplevel_page_tcd-google-maps') return;
  
  wp_enqueue_script('jquery-ui-core');
  wp_enqueue_script('jquery-ui-slider');

  //wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js');
  wp_enqueue_script('tcd-maps-angular', plugins_url('angular.js', __FILE__), array(), '1.4.8');
  wp_enqueue_script('tcd-maps-angular-slider', plugins_url('angular-slider.js', __FILE__), array('tcd-maps-angular'), '1.0.0');

  wp_enqueue_style('tcd-maps-admin', plugins_url('admin.css', __FILE__), array(), '1.0.0');
  wp_enqueue_style('tcd-maps-jquery-ui', plugins_url('jquery-ui.css', __FILE__), array(), '1.0.0');
}
...
function tcd_map_shortcode($opts){
  $mapId = esc_attr("map" . rand(100000, 999999));

  $address = !empty($opts['address']) ? sanitize_text_field($opts['address']) : '';
  $draggable = !empty($opts['draggable']) && strtolower($opts['draggable']) == 'true' ? 'true' : 'false';
  $controls = !empty($opts['controls']) && strtolower($opts['controls']) == 'true' ? 'true' : 'false';
  $controls_disabled = $controls == 'true' ? 'false' : 'true';
  $class = !empty($opts['class']) ? sanitize_text_field($opts['class']) : '';
  $style = !empty($opts['style']) ? sanitize_text_field($opts['style']) : '';
  if(wp_is_mobile()){
    $width = '100%';
  }else{
    $width = !empty($opts['width']) ? $opts['width'] : '100%';
  }
  $zoom = !empty($opts['zoom']) ? $opts['zoom'] : '18';
  if(wp_is_mobile()){
    $height = !empty($opts['height_m']) ? intval($opts['height_m']).'px' : '300px';
  }else{
    $height = !empty($opts['height']) ? intval($opts['height']).'px' : '400px';
  }
  $api = !empty($opts['api']) ? '?key=' . sanitize_text_field($opts['api']) : '';
  if(!isset($opts['theme'])){
    $opts['theme'] = '';
  }
  ...
  } else if( $opts['theme'] == 'justplaces' ){
    $theme = '[{"featureType":"road","elementType":"geometry","stylers":[{"visibility":"off"}]},{"featureType":"poi","elementType":"geometry","stylers":[{"visibility":"off"}]},{"featureType":"landscape","elementType":"geometry","stylers":[{"color":"#fffffa"}]},{"featureType":"water","stylers":[{"lightness":50}]},{"featureType":"road","elementType":"labels","stylers":[{"visibility":"off"}]},{"featureType":"transit","stylers":[{"visibility":"off"}]},{"featureType":"administrative","elementType":"geometry","stylers":[{"lightness":40}]}]';
  } else if( $opts['theme'] == 'cleancut' ){
    $theme = '[{"featureType":"road","elementType":"geometry","stylers":[{"lightness":100},{"visibility":"simplified"}]},{"featureType":"water","elementType":"geometry","stylers":[{"visibility":"on"},{"color":"#C6E2FF"}]},{"featureType":"poi","elementType":"geometry.fill","stylers":[{"color":"#C5E3BF"}]},{"featureType":"road","elementType":"geometry.fill","stylers":[{"color":"#D1D1B8"}]}]';
  } else if( $opts['theme'] == 'evenlighter' ){
  ...
  } else if( $opts['theme'] == 'greyscale' ){
    $theme = '[{"featureType":"all","elementType":"all","stylers":[{"saturation":-100},{"gamma":0.5}]}]';
  } else if( $opts['theme'] == 'lightgrey' ){
  ...
  } else if( $opts['theme'] == 'bluessence' ){
    $theme = '[{"featureType":"landscape.natural","elementType":"geometry.fill","stylers":[{"visibility":"on"},{"color":"#e0efef"}]},{"featureType":"poi","elementType":"geometry.fill","stylers":[{"visibility":"on"},{"hue":"#1900ff"},{"color":"#c0e8e8"}]},{"featureType":"road","elementType":"geometry","stylers":[{"lightness":100},{"visibility":"simplified"}]},{"featureType":"road","elementType":"labels","stylers":[{"visibility":"off"}]},{"featureType":"transit.line","elementType":"geometry","stylers":[{"visibility":"on"},{"lightness":700}]},{"featureType":"water","elementType":"all","stylers":[{"color":"#7dcdcd"}]}]';
  } else if( $opts['theme'] == 'bentley' ){
    $theme = '[{"featureType":"landscape","stylers":[{"hue":"#F1FF00"},{"saturation":-27.4},{"lightness":9.4},{"gamma":1}]},{"featureType":"road.highway","stylers":[{"hue":"#0099FF"},{"saturation":-20},{"lightness":36.4},{"gamma":1}]},{"featureType":"road.arterial","stylers":[{"hue":"#00FF4F"},{"saturation":0},{"lightness":0},{"gamma":1}]},{"featureType":"road.local","stylers":[{"hue":"#FFB300"},{"saturation":-38},{"lightness":11.2},{"gamma":1}]},{"featureType":"water","stylers":[{"hue":"#00B6FF"},{"saturation":4.2},{"lightness":-63.4},{"gamma":1}]},{"featureType":"poi","stylers":[{"hue":"#9FFF00"},{"saturation":0},{"lightness":0},{"gamma":1}]}]';
  } else if( $opts['theme'] == 'mybluewater' ){
  ...
  } else if( $opts['theme'] == 'flatmapwithlabels' ){
    $theme = '[{"featureType":"water","elementType":"all","stylers":[{"hue":"#7fc8ed"},{"saturation":55},{"lightness":-6},{"visibility":"on"}]},{"featureType":"water","elementType":"labels","stylers":[{"hue":"#7fc8ed"},{"saturation":55},{"lightness":-6},{"visibility":"off"}]},{"featureType":"poi.park","elementType":"geometry","stylers":[{"hue":"#83cead"},{"saturation":1},{"lightness":-15},{"visibility":"on"}]},{"featureType":"landscape","elementType":"geometry","stylers":[{"hue":"#f3f4f4"},{"saturation":-84},{"lightness":59},{"visibility":"on"}]},{"featureType":"landscape","elementType":"labels","stylers":[{"hue":"#ffffff"},{"saturation":-100},{"lightness":100},{"visibility":"off"}]},{"featureType":"road","elementType":"geometry","stylers":[{"hue":"#ffffff"},{"saturation":-100},{"lightness":100},{"visibility":"on"}]},{"featureType":"road","elementType":"labels","stylers":[{"hue":"#bbbbbb"},{"saturation":-100},{"lightness":26},{"visibility":"on"}]},{"featureType":"road.arterial","elementType":"geometry","stylers":[{"hue":"#ffcc00"},{"saturation":100},{"lightness":-35},{"visibility":"simplified"}]},{"featureType":"road.highway","elementType":"geometry","stylers":[{"hue":"#ffcc00"},{"saturation":100},{"lightness":-22},{"visibility":"on"}]},{"featureType":"poi.school","elementType":"all","stylers":[{"hue":"#d7e4e4"},{"saturation":-60},{"lightness":23},{"visibility":"on"}]}]';
  } else if( $opts['theme'] == 'fashionflat' ){
  ...
  } else if( $opts['theme'] == 'trainlinesbg' ){
    ...
    <script type="text/javascript">
      ...
      timer_$mapId = setInterval(function(){
        if(google && !loaded_$mapId){
          clearInterval(timer_$mapId);
          loaded_$mapId = true;

          var disabled = $controls_disabled;
